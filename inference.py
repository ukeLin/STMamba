...
def test_acdc(ckpt: str) -> None:
    ...
    dataset = ACDCDataset(base_dir="/root/autodl-tmp/data/ACDC/", split="test")
...
def test_synapse(ckpt: str) -> None:
    ...
    dataset = SynapseDataset(base_dir="/root/autodl-tmp/data/Synapse/test_vol_h5", split="test_vol")
    dataloader = DataLoader(dataset, batch_size=1, shuffle=False, num_workers=0)
    inference(
        model=model,
        dataloader=dataloader,
...
if __name__ == '__main__':
    # test_synapse(ckpt="log/STMamba-synapse/checkpoints/epoch=199-val_mean_dice=0.8549.ckpt")
    test_acdc(ckpt="log/msvm-unet-acdc/checkpoints/epoch=299-val_mean_dice=0.9236.ckpt")
